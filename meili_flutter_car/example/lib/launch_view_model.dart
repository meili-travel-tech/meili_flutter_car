...
class LaunchViewModel extends ChangeNotifier {
  ...
  /// Launches the MeiliCar SDK modally with the current settings.
  Future<void> launch() async {
    ...
    notifyListeners();
    try {
      await MeiliCar.open(_settings.toMeiliParams());
    } on Object catch (error) {
      _toasts.add('Failed to open MeiliCar: $error');
    } finally {
      _isLaunching = false;
      notifyListeners();
    }
  ...
  void _onEvent(MeiliCarEvent event) {
    if (event is MeiliCarBookingFlowEnded && _settings.showBookingToast) {
